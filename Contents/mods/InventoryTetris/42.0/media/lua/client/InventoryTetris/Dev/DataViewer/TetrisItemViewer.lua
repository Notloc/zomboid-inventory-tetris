...
TetrisItemViewer = ISPanel:derive("TetrisItemViewer");
...
function TetrisItemViewer:new(x, y, width, height)
    ...
    TetrisItemViewer.instance = o;
    ISDebugMenu.RegisterClass(self);
    return o;
end

function TetrisItemViewer:initialise()
    ...
    self.ok = ISButton:new(UI_BORDER_SPACING+1, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT - 1, btnWid, BUTTON_HGT, getText("IGUI_CraftUI_Close"), self, TetrisItemViewer.onClick);
...
function TetrisItemViewer:initList()
    ...
    local listBox = TetrisItemsListTable:new(0, 0, self.panel.width, self.panel.height - self.panel.tabHeight, self);
    listBox:initialise();
    ...
    for _,moduleName in ipairs(moduleNames) do
        ...
        if moduleName ~= "Moveables" then
            local cat1 = TetrisItemsListTable:new(0, 0, self.panel.width, self.panel.height - self.panel.tabHeight, self);
            cat1:initialise();
...
function TetrisItemViewer:prerender()
...
function TetrisItemViewer:onClick(button)
    if button.internal == "CLOSE" then
        self:close();
    end
end

function TetrisItemViewer:onSelectPlayer()
end

function TetrisItemViewer:setKeyboardFocus()
...
function TetrisItemViewer.OnOpenPanel()
    if TetrisItemViewer.instance then
        TetrisItemViewer.instance:setVisible(true)
        TetrisItemViewer.instance:addToUIManager()
