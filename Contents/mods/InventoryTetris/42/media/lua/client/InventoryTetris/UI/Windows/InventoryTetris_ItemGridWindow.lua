...
function ItemGridWindow:createChildren()
    ...
    local otherBumper = Joypad.LBumper
    local otherInvPanel = playerInv
    if playerInv.inventoryPane == self.inventoryPane then
        bumperButton = Joypad.LBumper
        otherBumper = Joypad.RBumper
        otherInvPanel = getPlayerLoot(self.playerNum)
    end
    ...
    NotlocControllerNode
        ...
        :setJoypadDownHandler(function(self, button)
            ...
            if button == otherBumper then
                setJoypadFocus(self.playerNum, otherInvPanel)
                return true
            end
